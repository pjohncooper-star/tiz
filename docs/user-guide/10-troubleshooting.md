...
| Symptom | Cause | Fix |
| --- | --- | --- |
| "Set missing intensity anchors before FIT export: …" | The workout uses a target that needs a threshold you haven't set | Set the named FTP, threshold pace, max heart rate, or race pace, then export again. |
