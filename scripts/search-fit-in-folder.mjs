...
const pathNeedle = (process.argv[3] ?? "courses").toLowerCase();
...
const fits = entries.filter(
  (e) => e.path.toLowerCase().endsWith(".fit") && e.path.toLowerCase().includes(pathNeedle)
);
console.log("FIT files under", pathNeedle, fits.length);
let found = 0;
for (const entry of fits) {
  ...
  console.log(JSON.stringify({
    ...
    hasCourse: (messages.courseMesgs?.length ?? 0) > 0,
  }));
}
console.log("trainerroad in", pathNeedle, found);
