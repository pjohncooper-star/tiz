...
type Props = {
  initialData: ThresholdHistoryEditorData;
};
...
function unitsRecord(
  settings: Record<PlanDiscipline, DisciplineUnitSettings>
): Record<string, DisplayUnit> {
  return {
    BIKE: settings.BIKE.displayUnit,
    RUN: settings.RUN.displayUnit,
    SWIM: settings.SWIM.displayUnit,
  };
}

export function ThresholdHistoryEditor({ initialData }: Props) {
  const [rows, setRows] = useState<ThresholdHistoryRow[]>(initialData.thresholds);
  const [signalPreferences, setSignalPreferences] = useState(
    initialData.signalPreferences
  );
  const [displayUnits, setDisplayUnits] = useState(() =>
    unitsRecord(initialData.unitSettings)
  );
    ...
    setSignalPreferences(
      (d.signalPreferences ?? []).map(
        (row: {
          ...
          roleSignals: row.roleSignals ?? null,
          effectiveDate: String(row.effectiveDate).slice(0, 10),
        })
      )
    );
  ...
  function formatValue(row: ThresholdHistoryRow) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-zinc-500">
        Add threshold and primary-metric changes with effective dates. Zone calculations
        use the values, primary metric, and any session-role metric overrides in effect on
        each activity date.
      </p>
                ...
                >
                  <div>
                    ...
                    <span className="text-zinc-500">
                      {" "}
                      · {row.discipline} primary:{" "}
                      {signalLabel(row.primarySignal as SignalType)}
                      {roleSummary ? ` · ${roleSummary}` : ""}
                    </span>
                  </div>
                  <button
                    type="button"
                    onClick={() => void removePrimaryEntry(row.id)}
      ...
      <Card title="Add primary metric change">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" onClick={() => void addPrimaryEntries()}>
      ...
      <Card title="Add historical entries">
        <div className="space-y-4">
          <div className="flex flex-wrap items-end gap-3">
            ...
            {isPace && (discipline === "RUN" || discipline === "SWIM") && (
              <div>
                ...
                <SegmentedControl
                  value={unit}
                  onChange={(next) => void setDisplayUnit(next)}
                  options={
                    discipline === "RUN"
                      ? [
            ...
            {draftRows.map((row) => (
              <div key={row.id} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                <Input
                  ...
                  value={row.effectiveDate}
                  onChange={(e) =>
                    updateDraftRow(row.id, { effectiveDate: e.target.value })
                  }
                />
                <Input
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" onClick={() => void addEntries()}>
              Save entries
            </Button>
          </div>
