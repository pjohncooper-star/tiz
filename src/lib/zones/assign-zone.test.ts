...
describe("zoneFromPaceSeconds", () => {
  const boundaries = zoneBoundariesFor("RUN", "PACE");

  it("maps pace via percent of threshold speed", () => {
    // threshold 300s/km; 300s = 100% speed → typically Z4
    assert.equal(
      zoneFromPaceSeconds(300, {
    ...
    // slower than threshold → lower zone
    assert.ok(
      zoneFromPaceSeconds(400, {
        thresholdPaceSeconds: 300,
        zoneBoundaries: boundaries,
        discipline: "RUN",
      }) < 4
  ...
  it("returns Z2 when threshold missing", () => {
    assert.equal(zoneFromPaceSeconds(300, { zoneBoundaries: boundaries }), 2);
  });
});
